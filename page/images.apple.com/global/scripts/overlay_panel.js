...
if(!c||(c&&c.lastIndexOf("../index.html")!==(c.length-1))){a="/"+a}return c+a},order:0,setOverlayShadowImageSrc:function(a){this._overlayShadowSrc=(a)?a:this.defaultOverlayShadowSrc();
...
},setDefaults:function(){this.defaultWidth=this.overlay.offsetWidth;this.padleft=parseInt(Element.getStyle(this.overlay,"margin-left").replace(/px/i,""),10);
this.padright=parseInt(Element.getStyle(this.overlay,"margin-right").replace(/px/i,""),10);
this.defaultHeight=this.overlay.offsetHeight;this.padtop=parseInt(Element.getStyle(this.overlay,"margin-top").replace(/px/,""),10);
this.padbottom=parseInt(Element.getStyle(this.overlay,"margin-bottom").replace(/px/,""),10)
...
this._popPosition=null;this.currentSectionEvent=a},superShouldAnimateContentChange:AC.ViewMaster.Viewer.prototype.shouldAnimateContentChange,shouldAnimateContentChange:function(c,b,a){if(AC.Detector.isMobile()){return false
...
var a=null;if(b){a=Element.cumulativeOffset(b.target||b.srcElement)}if(a){this.left=a.left
...
}this.top=this.top||document.body.getDimensions().height/2;this.padleft=parseInt(Element.getStyle(c,"margin-left").replace(/px/i,""),10);
this.padright=parseInt(Element.getStyle(c,"margin-right").replace(/px/i,""),10);
this.padtop=parseInt(Element.getStyle(c,"margin-top").replace(/px/,""),10);this.padbottom=parseInt(Element.getStyle(c,"margin-bottom").replace(/px/,""),10)
},windowSize:function(){var c=document.clientWidth||(document.documentElement.clientWidth||document.body.clientWidth);
...
if(AC.Detector.isMobile()){c=980;b=1212}return{width:c,height:b,x:a,y:d}},popPosition:function(b){if(!this._popPosition||b){var d,c=null;
var a=this.windowSize();d=a.x+(a.width-this.defaultWidth-this.padleft-this.padright)/2;
if(a.width<this.defaultWidth+this.padleft+this.padright){d=a.x-(this.padtop-this.closeBtnContainer.offsetWidth)
...
}},_closeOpenOverlay:function(b,e,a,d){if(!AC.Detector.isMobile()){var c=(100*this._width)/b;
