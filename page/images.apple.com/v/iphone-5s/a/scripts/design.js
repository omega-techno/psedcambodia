(function(){var C={_showOnScrolls:[],__onScroll:function v(a){this._showOnScrolls.forEach(function(b){b.__onScroll(a)
...
return AC.ShowOnScroll});AC.Ambient=AC.Class({__defaultOptions:{classNamePrefix:"ac-ambient-",delayBeforePlay:true,delayBeforeDidPlay:0,didPlayOnAnimationEnd:false,didPlayOnTransitionEnd:false,analytics:false},initialize:function ac_initialize(d,f,e){this._options=this.__mergeOptions(f);
...
AC.define("iphone/lib/ac_ambient/ac_ambient",function(){});AC.define("AC/Ambient",["require","AC/ShowOnScroll","iphone/lib/ac_ambient/ac_ambient"],function(b){b("AC/ShowOnScroll");
b("iphone/lib/ac_ambient/ac_ambient");return AC.Ambient});AC.define("iphone/shared/analytics/SectionEngagement",["require","AC/ShowOnScroll"],function(f){var h=f("AC/ShowOnScroll");
...
this._showOnScroll.setDelegate(this)}};return j});AC.define("design/analytics/builder",["require","iphone/shared/analytics/SectionEngagement"],function(d){var c=d("iphone/shared/analytics/SectionEngagement");
return function(a,b){b=b||{};a.forEach(function(h){b.id=h.getAttribute("data-track-visitor-engagement");
var g=new c(h,b);g.activate()})}});AC.define("flow/diff/Loader",["require","assetLoader/AssetLoader"],function(e){var h,f=e("assetLoader/AssetLoader");
...
AC.define("iphone/shared/sequence/builder",["require","flow/playerFactory"],function(f){var i=f("flow/playerFactory");
var g=AC.Environment.Feature;function h(){return typeof Object.defineProperties==="function"&&g.canvasAvailable()
}function j(a){AC.Element.removeClassName(a,"play");AC.Element.addClassName(a,"ended")
}return function(a,c){c=c||{};c.flow=c.flow||{};if(!h()){j(a);return null}var d=document.createElement("canvas");
var b=i(d,c.flow.keyframes,c.flow.imageUrlPattern,c.flow.manifestUrl,c.flow.options);
AC.Element.addClassName(d,"media");a.appendChild(d);d.on("canplaythrough",function(){AC.Element.addClassName(a,"ready")
});d.on("play",function(){AC.Element.removeClassName(a,"ended");AC.Element.removeClassName(a,"pause");
AC.Element.addClassName(a,"play")});d.on("pause",function(){AC.Element.removeClassName(a,"play");
AC.Element.addClassName(a,"pause")});d.on("ended",j.bind(undefined,a));return b
}});AC.define("iphone/shared/responsive/retinaPlus",["require"],function(d){var e=AC.Retina;
function f(a){var b,i,c,j=a.length;while(--j>-1){b=a[j].src;i=b.split("/");c=e.sharedInstance().bestSrc(i[i.length-1]);
i[i.length-1]=c;b=i.join("../../index.html");a[j].src=b}return true}return f});window.DEBUG=false;
AC.define("design/sequence/hero/builder",["require","iphone/shared/sequence/builder","iphone/shared/responsive/retinaPlus"],function(e){var f=e("iphone/shared/sequence/builder");
var g=e("iphone/shared/responsive/retinaPlus");function h(a){var j="../../105/media/us/iphone-5s/2013/design/hero/index.html";
var l="j";var b=[j+"hero_start__keyframe."+l+"pg",j+"hero_start__posterframe."+l+"pg"];
var c="p";var d=f(a,{flow:{keyframes:b,imageUrlPattern:j+"hero_start__###."+c+"ng",manifestUrl:j+"hero_start__manifest.json",options:{}}});
g(AC.Element.selectAll("img",a));return d}return h});AC.define("design/bootstrap",["require","AC/Ambient","design/analytics/builder","design/sequence/hero/builder"],function(w){var r=AC.Element;
var t=AC.Environment;var x=w("AC/Ambient");var m=w("design/analytics/builder");
var s=w("design/sequence/hero/builder");var u;var o;var p;u=r.select(".flushrow.hero");
if(u){r.addClassName(u,"show")}o=r.select(".flushrow.touchid .infograph");new x.Scroll(o,{playOnVisitorEngaged:true,showOnScrollOptions:{threshold:1}});
if(!t.Feature.isCSSAvailable("transition")){r.addClassName(o,"ac-ambient-play play-end")
}var v=r.select(".flushrow.touchid .infograph figure");r.addEventListener(v,"transitionend",n,true);
r.addEventListener(v,"webkitTransitionEnd",n,true);function n(){r.addClassName(o,"play-end")
}if(u){p=r.select(".hero",u);if(p){var q=s(p);if(q){q.on("canplaythrough",function(){setTimeout(q.play.bind(q),500)
})}}}m(AC.Element.selectAll("[data-track-visitor-engagement]"),{decimals:1});return{}
});