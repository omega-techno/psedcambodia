...
}a=c+a.replace(/^\//,"");a=a.replace(/(^.+)(\/105\/)/,"$1/");return a}});(function(l,h){var j="",k,i,m;
if(l.addEventListener){k="addEventListener"}else{k="attachEvent";j="on"}m="onwheel" in h.createElement("div")?"wheel":h.onmousewheel!==undefined?"mousewheel":"DOMMouseScroll";
...
var c=function(){this.removeTransition.bind(this,a);AC.Element.removeVendorPrefixEventListener(a,"transitionEnd",c)
}.bind(this);if(b){this.addOpacityTransition(a);AC.Element.addVendorPrefixEventListener(a,"transitionEnd",c)
}else{this.removeTransition(a)}window.requestAnimationFrame(function(){AC.Element.setStyle(a,{opacity:1,zIndex:1001})
}.bind(this))}},hidePosterframe:function(b){if(this.posterframeElement()){var a=this.posterframeElement();
...
}else{this.removeTransition(a)}window.requestAnimationFrame(function(){AC.Element.setStyle(a,{opacity:1,zIndex:1002})
}.bind(this))}},hideEndstate:function(b){if(this.endstateElement()){var a=this.endstateElement();
...
var d=this.basePath()+this.filename()+"_###."+this.flowExtension();var b=this.basePath().replace(/^https?:\/\/[^\/]+\//i,"../index.html")+this.filename()+"_manifest."+c;
...
this._staticClass="static";this._outgoingInteractionType="scroll"}var j=h.prototype=new i();
j.getScene=function(a){return this._scenes[a]};j.currentSceneIndex=function(){return this._currentSceneIndex
};j.isLocked=function(){return this._locked};j.setLocked=function(a){if(a===true){this.trigger("lock")
}else{if(a===false){this.trigger("unlock")}}this._locked=a;return this};j.unlock=function(){return this.setLocked(false)
...
var c=this.getScene(e);var d=this.getScene(this._currentSceneIndex);if(this._storyStatic===false){a=this.__hideMediaContainer();
this._player.pause()}else{a=this.__hideSceneContainer(d._scenes[this._currentSceneIndex],this._currentSceneIndex)
}a=a.then(function(){if(this._currentSceneIndex===8&&e===7){c.setPlaybackRate(c.getPlaybackRate()*-1)
}if(e===7||e===8){c.play()}this.__showSceneContainer(c._scenes[e],e);if(this._storyStatic===false){this._storyStatic=true
...
this.trigger("didShow",{outgoingIndex:a,incomingIndex:c,incomingScene:this.getScene(c)._scenes[c],incomingInteractionType:b,outgoingScene:this.getScene(a)._scenes[a],outgoingInteractionType:this._outgoingInteractionType});
this._outgoingInteractionType=b};return h});AC.define("overview/shared/story/observer/Scene",[],function(){function b(a,d){this._player=a;
...
}};d.initInteraction=function(a){this._interactionStart=a||+new Date()};d.queueInteraction=function(s,c,t,a,q){var r=+new Date();
var n=((r-this._interactionStart)/1000).toFixed(1);var p=AC.Tracking.pageName()+" - "+c.toLowerCase()+" - section "+a;
var o=AC.Tracking.pageName()+" - "+t.toLowerCase()+" - section "+q;var b={prop1:s,prop3:o,prop2:p,prop35:n};
this._trackingQueue.push(b)};d.trackProps=function(a){if(typeof a.prop3!=="string"){return
...
}.bind(this))}d.prototype={onDidShow:function(a){if(this._previousIndex>=this._footerIndex&&a.incomingIndex>=this._footerIndex){return
}if(window.analyticsIntroStartTime!==undefined){this.analyzer.initInteraction(window.analyticsIntroStartTime);
this.analyzer.queueInteraction(a.interactionType,a.outgoingScene.id,a.incomingScene.id,a.outgoingIndex,a.incomingIndex);
window.analyticsIntroStartTime=undefined}else{this.analyzer.queueInteraction(a.interactionType,a.outgoingScene.id,a.incomingScene.id,a.outgoingIndex,a.incomingIndex)
}this._previousIndex=a.incomingIndex;this.analyzer.initInteraction();if(this._analyzerTimeout!==undefined){clearTimeout(this._analyzerTimeout)
...
while(!AC.Element.matchesSelector(b,this._elementSelector)){b=b.parentNode}if(!AC.Element.hasClassName(b,"active")&&this._elements.indexOf(b)!==-1){this._story.show(this._elements.indexOf(b),"bubble-click")
...
AC.Element.removeClassName(document.body,this._storyStartedClass);return g.prototype.showLastFrame.apply(this,arguments)
};i.play=function(){var a=new f();if(this.getPlaybackRate()<0){AC.Element.removeClassName(document.body,this._storyStartedClass);
return a.resolve()}else{return g.prototype.play.apply(this,arguments)}};return h
});AC.define("overview/shared/scene/SceneFooter",["require","defer/Deferred","overview/shared/scene/Scene"],function(i){var j=i("defer/Deferred");
...
J.onWillShow(n);y.onWillShow(n);Q.onWillShow(n)});j.pauseController=K;window.story=j;
return j}});AC.define("ipad/shared/section_engagement/track_section_engagement",["require"],function(d){var c=Class.create({__defaultOptions:{onlyTrackOnce:true,id:"",minimumDuration:1,threshold:0.75,debug:false,decimals:1},initialize:function(a,b){if(typeof b!=="object"){b={}
...
}new c(b,g)})})});AC.define("overview/desktop/bootstrap",["require","ipad/shared/cnamer/cnameURL","ipad/shared/utils/wheel","ipad/shared/utils/dom","overview/desktop/controller/Input","overview/desktop/experience/builder","overview/desktop/storyboard/intro","overview/desktop/storyboard/story","overview/desktop/experience/intro","overview/desktop/experience/story","overview/shared/story/controller/Media","overview/shared/intro/builder","overview/shared/story/builder","ipad/shared/section_engagement/track_section_engagement","ipad/shared/section_engagement/track_section_engagement"],function(D){window.DEBUG=false;
window.FALLBACK_TIMEOUT=10;var G=D("ipad/shared/cnamer/cnameURL");var z=window.MEDIA_BASE_PATH||"/105/media/us/ipad-air/2013/0be12b9f-265c-474c-a0cc-d3c4c304c031/overview/desktop/";
z=G(z);D("ipad/shared/utils/wheel");var u=D("ipad/shared/utils/dom");var A=D("overview/desktop/controller/Input");
var t=D("overview/desktop/experience/builder");var C=D("overview/desktop/storyboard/intro");
var y=D("overview/desktop/storyboard/story");var I=D("overview/desktop/experience/intro");
var H=D("overview/desktop/experience/story");var v=D("overview/shared/story/controller/Media");
var x=D("overview/shared/intro/builder");var F=D("overview/shared/story/builder");
var E=t(I,C,z);var s=t(H,y,z);C.mediaElement=E[0];y.mediaElement=s[0];var B=s[1].canEnhance();
var w=s[1].getExperience();function J(){window.scrollTo(0,0)}if(B){AC.Element.addClassName(document.body,"enhanced");
AC.Element.addClassName(document.documentElement,"overflow-hidden");window.addEventListener("scroll",J)
}AC.onDOMReady(function(){if(B){var i=[];var e=new v(y.mediaElement,C.mediaElement,w);
var f=new A();y.scenes.forEach(function(j){i.push(j.endTime)});var d=y.mediaElement;
var a={experience:w,startScene:1,sceneIndex:{wireless:3,ios:5,smartcover:6,footer:7},enableFallback:false};
function g(){var j=b.currentSceneIndex();b=F(d,y.scenes,i,f,a);if(j>b.numScenes()-1){b.show(b.numScenes()-1)
}else{b._currentSceneIndex=j}}var b=F(d,y.scenes,i,f,a);var h=false;u.bindThrottledCallback("resize",function(){if(b.isLocked()&&h===false){h=true;
b.on("didShow",function(){g();h=false})}else{if(h===false){g()}}});var c=x(C,y);
c._mediaElement.setPosterframeElement(AC.Element.getElementById("intro-posterframe"));
e.on("startstory",function(){window.analyticsIntroStartTime=+new Date();c.hide();
b.show();f.enable();f.on("backward",function(j){b.show(b.currentSceneIndex()-1,j.interactionType)
});f.on("forward",function(j){b.show(b.currentSceneIndex()+1,j.interactionType)
});f.on("beginning",function(j){b.show(0,j.interactionType)});f.on("end",function(j){b.show(b.numScenes()-1,j.interactionType)
});AC.Element.addClassName(document.body,"intro-finished")});e.on("startintro",function(){var j=AC.Element.getElementById("intro-posterframe");
AC.Element.addClassName(j,"transition-in");AC.Element.addVendorPrefixEventListener(j,"transitionEnd",function(){AC.Element.addClassName(document.body,"media-ready");
window.setTimeout(c.play.bind(c),0.4*1000)})});e.on("fallback",function(){f.disable();
AC.Element.removeClassName(document.body,"enhanced");AC.Element.removeClassName(document.documentElement,"overflow-hidden");
window.removeEventListener("scroll",J);D("ipad/shared/section_engagement/track_section_engagement")
})}else{D("ipad/shared/section_engagement/track_section_engagement")}})});